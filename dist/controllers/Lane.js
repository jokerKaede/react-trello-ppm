...
class Lane extends _react.Component {
  constructor(...args) {
    ...
    (0, _defineProperty2.default)(this, "state", {
      ...
      firstCardIndexInViewbox: 0,
      cardHeight: this.props.cardStyle.height || minCardHeight,
      cardsInDomCount: minCardsInDomCount
    });
    (0, _defineProperty2.default)(this, "handleScroll", evt => {
      clearTimeout(this.laneScrollTimeout);
      this.laneScrollTimeout = setTimeout(() => {
        const node = evt.target;
        const elemScrollPosition = node.scrollHeight - node.scrollTop - node.clientHeight;
        let cardHeight = this.props.cardStyle.height || minCardHeight;
        let cardsInDomCount = Math.ceil(node.clientHeight / cardHeight) + 2;
        let firstCardIndexInViewbox = parseInt(node.scrollTop / cardHeight);

        if (cardHeight !== this.state.cardHeight || cardsInDomCount !== this.state.cardsInDomCount || firstCardIndexInViewbox !== this.state.firstCardIndexInViewbox) {
          this.setState({
            cardHeight: cardHeight,
            cardsInDomCount: Math.ceil(node.clientHeight / cardHeight) + 2,
            firstCardIndexInViewbox: parseInt(node.scrollTop / cardHeight)
          });
        }

        this.props.updateLaneScrollPosition(this.props.boardId, this.props.id, node.scrollTop);
        const onLaneScroll = this.props.onLaneScroll;

        if (elemScrollPosition <= 1 && onLaneScroll && !this.state.loading) {
          // some browser scroll to bottom position is 0~1, should save some buffer
    ...
    (0, _defineProperty2.default)(this, "onDragEnd", (laneId, result) => {
      ...
      if (addedIndex != null) {
        const newCard = _objectSpread({}, (0, _cloneDeep.default)(payload), {
          laneId
        });
    ...
    (0, _defineProperty2.default)(this, "renderDragContainer", isDraggingOver => {
      const _this$props = this.props,
            ...
            removeCardDomOutsideViewbox = _this$props.removeCardDomOutsideViewbox,
            t = _this$props.t;
      const _this$state = this.state,
            addCardMode = _this$state.addCardMode,
            collapsed = _this$state.collapsed,
            firstCardIndexInViewbox = _this$state.firstCardIndexInViewbox,
            cardHeight = _this$state.cardHeight,
            cardsInDomCount = _this$state.cardsInDomCount;
      let showableCards = collapsed ? [] : [...cards];
      let laneStyle = {
        height: showableCards.length * (cardHeight || 0)
      };

      if (!collapsed && cardsInDomCount && showableCards.length > cardsInDomCount) {
        if (firstCardIndexInViewbox > 1) {
          laneStyle.paddingTop = (firstCardIndexInViewbox - 1) * cardHeight;
          showableCards = showableCards.slice(firstCardIndexInViewbox - 1);

          if (showableCards.length > cardsInDomCount) {
            showableCards = showableCards.slice(0, cardsInDomCount);
          }
        } else {
          if (showableCards.length > cardsInDomCount) {
            showableCards = showableCards.slice(0, cardsInDomCount);
      ...
      let cardList = this.sortCards(showableCards, laneSortFunction).map((card, idx) => {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (!(0, _isEqual.default)(this.props.cards, nextProps.cards) || this.props.currentPage !== nextProps.currentPage) {
      this.setState({
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.container && this.props.scrollPosition && prevState.firstCardIndexInViewbox === this.state.firstCardIndexInViewbox && prevState.loading === this.state.loading && prevState.currentPage === this.state.currentPage) {
      this.container.scrollTop = this.props.scrollPosition;
    }
  }
  ...
  render() {
    ...
    const _this$props2 = this.props,
          ...
          otherProps = (0, _objectWithoutProperties2.default)(_this$props2, ["id", "cards", "collapsibleLanes", "components", "onLaneClick", "onLaneScroll", "onCardClick", "onCardAdd", "onBeforeCardDelete", "onCardDelete", "onLaneDelete", "onLaneUpdate", "onCardMoveAcrossLanes", "scrollPosition"]);
...
Lane.propTypes = {
  ...
  updateLaneScrollPosition: _propTypes.default.func,
